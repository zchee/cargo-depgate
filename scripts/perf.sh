...
# The script's real standard error, captured before any measurement starts folding its own
# stderr into a log with `2>&1` and then printing that log as the gate report. A diagnostic
# that must stay out of the report entirely -- the feature-aware peak RSS figure, which the
# AC-P5 ruling deliberately does not gate on (plan §6, 2026-08-31) -- is written here, so
# no measurement can turn it into something that looks like a gate line.
exec 3>&2
